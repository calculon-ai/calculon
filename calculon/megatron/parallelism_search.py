...
class ParallelismSearch(calculon.CommandLine):
  NAME = 'megatron-parallelism-search'
  ALIASES = ['mps']

  @staticmethod
  def create_parser(subparser):
    sp = subparser.add_parser(
      ParallelismSearch.NAME, aliases=ParallelismSearch.ALIASES,
      help='run a search to find the optimal megatron execution')
    sp.set_defaults(func=ParallelismSearch.run_command)
  ...
  def run_command(logger, args):
    ...
    if not args.debug:
      ...
      logger.info(f'Wrote to {args.stats}')

    return 0

calculon.CommandLine.register(ParallelismSearch)
